...
//			Assert.assertEquals(str,td_reader("signup_exist_no_msg"));
	...
	/*TC_011 -verify incorrect referral code*/
	@When("Enter incorrect referral code")
	public static void enter_incorrect_referral_code() throws Exception  {
		try {
			browser_wait(5000);
			click("signup_referral_code");
			driver.findElement(By.xpath(OR_reader("signup_referral_code"))).sendKeys("TEST");
			click("signup_apply");
		} catch (Exception e) {
			e.printStackTrace();
			takeScreenShot("signup_negative_enter_incorrect_referral_code");
		}		
	}
	
	@Then("Validation message displayed")
	public static void incorrect_referral_validation() throws Exception  {
		try {
			str= driver.findElement(By.xpath(OR_reader("signup_referral_code_message"))).getText();
			Assert.assertEquals(str,"Invalid referral code");
			System.out.println("signup negative");
			browser_close();
		} catch (Exception e) {
			e.printStackTrace();
			takeScreenShot("signup_negative_incorrect_referral_validation");
		}		
	}
}
